...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None):
        ...
        openPrevAnnotation = action('&Load Prev\nAnnotations', self.openPrevAnnotation,
                'Ctrl+v', 'open-prev-annotation', u'Load Prev Annotations')
        ...
        save = action('&Save', self.saveFileDefault,
    ...
    def openPrevAnnotation(self, _value=False):
        if self.filePath is None:
            return
        path = os.path.dirname(u(self.filePath))\
                if self.filePath else '.'
        if self.usingPascalVocFormat:
            if self.itemsToShapes:
                self.errorMessage(u'Labels already exist',
                        u'Remove all existing labels to load the labels of the previous frame.')
                return
            formats = ['*.%s' % str(fmt).lower()\
                    for fmt in QImageReader.supportedImageFormats()]
            filters = "Open Annotation XML file (%s)" % \
                    ' '.join(formats + ['*.xml'])
            sortedDir = sorted(os.listdir(path))
            lastXML = None
            for root, dirs, files in os.walk(path):
                for file in files:
                    if os.path.join(os.path.dirname(u(self.filePath)), file) == self.filePath:
                        filename = lastXML
                        print lastXML
                        if lastXML == None:
                    ...
                    if file.endswith(".xml"):
                        lastXML = os.path.join(os.path.dirname(u(self.filePath)), file)
    ...
    def saveFileDefault(self, _value=False):
